...
int main(int, char*[]) {
  ...
  Graph g(std::begin(used_by), std::end(used_by), std::size(name));
  ...
  // and the order with files that can be compiled in parallel
  {
    ...
    std::vector<int> time(std::size(name), 0);
    for (auto i = make_order.begin(); i != make_order.end(); ++i) {
      ...
      if (in_degree(*i, g) > 0) {
        ...
        for (auto [j, j_end] = in_edges(*i, g); j != j_end; ++j)
              ...
              << std::endl;
    {
      for (auto [i, iend] = vertices(g); i != iend; ++i)
        std::cout << "time_slot[" << name[*i] << "] = " << time[*i]
                  << std::endl;
    }
  }
  std::cout << std::endl;
