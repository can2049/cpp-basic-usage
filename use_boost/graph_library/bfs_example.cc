...
class bfs_time_visitor : public boost::default_bfs_visitor {
 ...
 public:
  ...
  void discover_vertex(Vertex u, [[maybe_unused]] const Graph& g) const {
...
int main() {
  ...
  std::vector<boost::graph_traits<graph_t>::vertices_size_type> discover_order(
      N);
  boost::integer_range<int> range(0, N);
  std::copy(range.begin(), range.end(), discover_order.begin());
  std::sort(discover_order.begin(), discover_order.end(),
            boost::indirect_cmp<dtime_pm_type, std::less<Size> >(dtime_pm));

  std::cout << "order of discovery: ";
  for (int i = 0; i < N; ++i) std::cout << name[discover_order[i]] << " ";
  std::cout << std::endl;
