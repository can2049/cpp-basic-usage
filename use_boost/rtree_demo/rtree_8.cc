...
// https://live.boost.org/doc/libs/1_85_0/libs/geometry/doc/html/geometry/spatial_indexes/rtree_examples/index_stored_in_shared_memory_using_boost_interprocess.html
...
int main(int argc, char* argv[]) {
  ...
  if (argc == 1) {
    ...
    Rtree* tree = segment.construct<Rtree>("Rtree")(
        Par(), I(), E(), Alloc(segment.get_segment_manager()));
    ...
    std::cout << "[(" << bg::get<0>(bb.min_corner()) << ", "
              << bg::get<1>(bb.min_corner()) << ")("
              << bg::get<0>(bb.max_corner()) << ", "
              << bg::get<1>(bb.max_corner()) << ")]\n";
    ...
    if (segment.find<Rtree>("Rtree").first) return 1;
  ...
  // Child process
    ...
    std::cout << "Child: Querying for objects intersecting box = [(45, 45)(55, "
                 "55)]\n";
    ...
    BOOST_FOREACH (B const& b, result) {
      std::cout << "[(" << bg::get<0>(b.min_corner()) << ", "
                << bg::get<1>(b.min_corner()) << ")("
                << bg::get<0>(b.max_corner()) << ", "
                << bg::get<1>(b.max_corner()) << ")]\n";
    }
    std::cout << "\n";
